...
/**
 * 自定义选项标签
 * author：T-Baymax on 2018/1/27 14:00
...
public class LabelsView extends ViewGroup implements View.OnClickListener {
    ...
    //用于保存label数据的key
    private static final int KEY_DATA = com.donkingliang.labels.R.id.tag_key_data;
    //用于保存label位置的key
    private static final int KEY_POSITION = com.donkingliang.labels.R.id.tag_key_position;
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray mTypedArray = context.obtainStyledAttributes(attrs, com.donkingliang.labels.R.styleable.labels_view);
            int type = mTypedArray.getInt(com.donkingliang.labels.R.styleable.labels_view_selectType, 1);
            mSelectType = SelectType.get(type);

            mMaxSelect = mTypedArray.getInteger(com.donkingliang.labels.R.styleable.labels_view_maxSelect, 0);
            mTextColor = mTypedArray.getColorStateList(com.donkingliang.labels.R.styleable.labels_view_labelTextColor);
            mTextSize = mTypedArray.getDimension(com.donkingliang.labels.R.styleable.labels_view_labelTextSize,
                    sp2px(context, 14));
            mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(
                    com.donkingliang.labels.R.styleable.labels_view_labelTextPaddingLeft, 0);
            mTextPaddingTop = mTypedArray.getDimensionPixelOffset(
                    com.donkingliang.labels.R.styleable.labels_view_labelTextPaddingTop, 0);
            mTextPaddingRight = mTypedArray.getDimensionPixelOffset(
                    com.donkingliang.labels.R.styleable.labels_view_labelTextPaddingRight, 0);
            mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(
                    com.donkingliang.labels.R.styleable.labels_view_labelTextPaddingBottom, 0);
            mLineMargin = mTypedArray.getDimensionPixelOffset(com.donkingliang.labels.R.styleable.labels_view_lineMargin, 0);
            mWordMargin = mTypedArray.getDimensionPixelOffset(com.donkingliang.labels.R.styleable.labels_view_wordMargin, 0);
            mLabelBgResId = mTypedArray.getResourceId(com.donkingliang.labels.R.styleable.labels_view_labelBackground, 0);
            mTypedArray.recycle();
        }
    }
        ...
        if (labels != null) {
            ...
            for (int i = 0; i < size; i++) {
                if(i==0){
                    ...
                    label.setTag(KEY_POSITION,i);
                    addView(label);
                    label.setText(provider.getLabelText(label,i,data));
                }else {
